...
<html>
	<head>
		<meta charset="utf-8">
		<title>爱智网 - ${sessionScope.user.userName }的提问</title>
					...
					<div class="panel panel-default" id="userleft">
						<div class="panel-heading space-tabs-menu">													
							<ul class="myspace-tab-list">
				               <li class="active"><a href="<%=basePath %>userpost/quePost.action">我的提问</a></li>
				                <li ><a href="<%=basePath %>userreply/myReply.action">我的回答</a></li>
				                
			                </ul>
			                <div class="dot-curr"></div>
						</div>
						<div class="panel-body">
						<c:if test="${ques.recordCount==0 }">
						<div class="media">
						您还没有提问记录，快去提问吧
						</div>
						</c:if>
						<c:if test="${ques.recordCount!=0 }">
						<c:forEach items="${ques.recordList }" var="oneques">
						<div class="media">
							  ...
							  <div class="media-body">
							   <!--  <h4 class="media-heading user-head"><a href="javascrip:;">心悦君兮君不知</a>提问</h4> -->
							    <div class="user-comment">
							    	
							    	<span class="fa fa-edit"></span>&nbsp&nbsp
							  	 	<a href="javascrip:;">${oneques.postTitle}</a>
							    </div>
							    <div class="media-bottom">
							  		<time>提问时间：${oneques.postTime}</time>
							    	<div class="course-from label-tag">${oneques.labelName }</div>
								  </div>
							  </div>	
							...
							<nav>
							  ...
							  <ul class="pagination">
							  	...
							  	<li><a href="<%=basePath %>userpost/quePost.action?currentPage=${ques.beginPageIndex }">首页</a></li>
							  	
							  	<c:if test="${ques.currentPage-1<1}">
								...
								<c:if test="${ques.currentPage-1>=1}">
									<li><a
										href="<%=basePath %>userpost/quePost.action?currentPage=${ques.currentPage-1}"
										aria-label="Previous"> <span aria-hidden="false">上一页</span>
									</a>
									</li>
								</c:if>
							      ...
							      <c:forEach var="page" begin="${ques.beginPageIndex}" end="${ques.endPageIndex}">
							    	<c:if test="${page==ques.currentPage }">
							    	 <li class="active"><a href="<%=basePath %>userpost/quePost.action?currentPage=${page}">${page}</a></li>
							    	</c:if>
							    	<c:if test="${page!=ques.currentPage }">
							    	<li><a href="<%=basePath %>userpost/quePost.action?currentPage=${page}">${page}</a></li>
							    	</c:if>
							     </c:forEach>
							     <c:if test="${ques.currentPage+1>ques.endPageIndex}">
									<li class="disabled"><a href="javascript:void(0)"
							    ...
							    <li><a href="<%=basePath %>userpost/quePost.action?currentPage=${ques.endPageIndex}">尾页</a></li>
